...
import { IconTrendingUp, IconTrendingDown, IconCoin, IconClockHour4 } from '@tabler/icons-react';
...
const STATS = [
    ...
    },
    {
        ...
        color: '#3D5EE1',
        bg: '#eef2ff',
        gradient: 'linear-gradient(135deg,#3D5EE1,#6C49EF)',
    },
    {
        label: 'Pending Dues',
        value: '$12,450',
        ...
        {STATS.map((s, i) => (
            <div key={i} style={{ background: 'white', borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.07)', border: '1px solid #f1f5f9', padding: '20px', overflow: 'hidden', position: 'relative', transition: 'transform 0.2s, box-shadow 0.2s' }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    ...
                    <span style={{ fontSize: 12, fontWeight: 700, padding: '4px 10px', borderRadius: 20, background: s.trend === 'up' ? (s.label === 'Total Expenses' ? '#fee2e2' : '#d1fae5') : '#fef3c7', color: s.trend === 'up' ? (s.label === 'Total Expenses' ? '#dc2626' : '#059669') : '#d97706' }}>
                        {s.change}
                    </span>
                </div>
                <div style={{ fontWeight: 900, fontSize: 24, color: '#1e1b4b', marginBottom: 4 }}>{s.value}</div>
...
export default AccountantStatCards;
