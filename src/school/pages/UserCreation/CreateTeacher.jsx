...
import {
    ...
    IconCurrencyRupee, IconBook, IconLeaf, IconWorld
...
const SectionCard = ({ icon: Icon, title, color, bg, children }) => (
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
            <div onClick={() => ref.current.click()} style={{
                ...
                background: previewUrl ? 'transparent' : '#fef3c7',
                border: '3px dashed #fcd34d', cursor: 'pointer',
            ...
            >
                {previewUrl
                    ...
                    : <IconCamera size={30} color="#f59e0b" />
                }
            </div>
            <p style={{ fontSize: 11, color: '#9ca3af', margin: 0, textAlign: 'center', lineHeight: 1.5 }}>
                Click to upload photo<br /><span>JPG, PNG (max 4MB)</span>
            </p>
...
const CreateTeacher = () => {
    ...
    const [professional, setProfessional] = useState({
        employee_id: '', department: '', designation: '', subject: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.response?.data?.detail || 'Failed to create teacher. Please check the details.');
        } finally {
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #fffbeb 0%, #fefce8 100%)', paddingBottom: 60 }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    ...
                    <div>
                        <h1 style={{ fontWeight: 800, fontSize: 20, color: '#1e1b4b', margin: 0 }}>Add New Teacher</h1>
                        <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>Fill in all details to register a new teaching staff member</p>
                    </div>
                </div>
                <div style={{ display: 'flex', gap: 10 }}>
                    <button type="button" onClick={() => navigate(-1)} style={{
                        padding: '10px 22px', borderRadius: 10, border: '1.5px solid #e5e7eb',
                        background: 'white', fontWeight: 600, cursor: 'pointer', color: '#6b7280', fontSize: 14
                    }}>Cancel</button>
                    <button form="teacher-form" type="submit" disabled={isSubmitting || success} style={{
                        padding: '10px 26px', borderRadius: 10, border: 'none',
                        background: 'linear-gradient(135deg, #f59e0b, #d97706)',
                        color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: 14,
                        display: 'flex', alignItems: 'center', gap: 8,
                        boxShadow: '0 6px 18px rgba(245,158,11,0.35)', opacity: isSubmitting ? 0.7 : 1
                    }}>
            ...
            <form id="teacher-form" onSubmit={handleSubmit} style={{ maxWidth: 1100, margin: '32px auto', padding: '0 24px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                ...
                {success && (
                    <div style={{ background: '#d1fae5', border: '1.5px solid #34d399', borderRadius: 14, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 12, color: '#065f46', fontWeight: 600 }}>
                        <IconCheck size={22} /> Teacher added successfully! Redirecting...
                    </div>
                )}
                {error && (
                ...
                <SectionCard icon={IconUser} title="Personal Information" color="#f59e0b" bg="#fef3c7">
                    <div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 24, marginBottom: 20 }}>
                        <PhotoUpload value={photo} onChange={setPhoto} />
                        <Grid cols={3}>
                            <Input label="Full Name" required icon={IconUser} placeholder="e.g. Priya Sharma" value={personal.name} onChange={set(setPersonal)('name')} />
                    ...
                    <Grid cols={4}>
                        ...
                        <Input label="Email Address" required icon={IconMail} type="email" placeholder="teacher@school.com" value={personal.email} onChange={set(setPersonal)('email')} />
                ...
                <SectionCard icon={IconLock} title="Login Credentials" color="#6366f1" bg="#eef2ff">
                    <Grid cols={2}>
                        <Input label="Username" required icon={IconId} placeholder="priya_sharma" value={account.username} onChange={set(setAccount)('username')} />
                        <Input label="Temporary Password" required icon={IconLock} type="password" placeholder="••••••••" value={account.password} onChange={set(setAccount)('password')} hint="Teacher will be prompted to change on first login." />
                    </Grid>
                </SectionCard>

                {/* ─── 3. Professional Details ─── */}
                <SectionCard icon={IconBook} title="Professional Details" color="#0ea5e9" bg="#e0f2fe">
                    <Grid cols={4}>
                        ...
                        <Input label="Department" placeholder="e.g. Science" value={professional.department} onChange={set(setProfessional)('department')} />
                        <Input label="Designation" placeholder="e.g. Senior Teacher" value={professional.designation} onChange={set(setProfessional)('designation')} />
                        <Input label="Primary Subject" placeholder="e.g. Mathematics" value={professional.subject} onChange={set(setProfessional)('subject')} />
                    </Grid>
                    <div style={{ height: 16 }} />
                    <Grid cols={4}>
                        <Input label="Highest Qualification" placeholder="e.g. M.Sc, B.Ed" value={professional.qualification} onChange={set(setProfessional)('qualification')} />
                        <Input label="Years of Experience" type="number" placeholder="e.g. 8" value={professional.experience_years} onChange={set(setProfessional)('experience_years')} />
                        ...
                        <Select label="Contract Type" value={professional.contract_type} onChange={set(setProfessional)('contract_type')}>
                            ...
                            <option>Permanent</option><option>Contractual</option><option>Guest / Part-time</option>
                        </Select>
                    </Grid>
                </SectionCard>
                ...
                <SectionCard icon={IconCurrencyRupee} title="Payroll" color="#10b981" bg="#d1fae5">
                    <Grid cols={3}>
                        ...
                        <Input label="Basic Salary (₹)" type="number" icon={IconCurrencyRupee} placeholder="e.g. 45000" value={payroll.basic_salary} onChange={set(setPayroll)('basic_salary')} />
                    ...
                    <Grid cols={2}>
                        ...
                        <Input label="Gratuity (₹)" type="number" placeholder="e.g. 5000" value={payroll.gratuity} onChange={set(setPayroll)('gratuity')} />
                    </Grid>
                </SectionCard>
                ...
                <SectionCard icon={IconLeaf} title="Leave Allocation" color="#8b5cf6" bg="#f3e8ff">
                    <Grid cols={4}>
                        <Input label="Medical Leave (days)" type="number" placeholder="e.g. 12" value={leaves.medical_leave} onChange={set(setLeaves)('medical_leave')} />
                        <Input label="Casual Leave (days)" type="number" placeholder="e.g. 10" value={leaves.casual_leave} onChange={set(setLeaves)('casual_leave')} />
                        <Input label="Maternity Leave (days)" type="number" placeholder="e.g. 180" value={leaves.maternity_leave} onChange={set(setLeaves)('maternity_leave')} />
                        <Input label="Sick Leave (days)" type="number" placeholder="e.g. 7" value={leaves.sick_leave} onChange={set(setLeaves)('sick_leave')} />
                    </Grid>
                </SectionCard>
                ...
                <SectionCard icon={IconMapPin} title="Address" color="#0ea5e9" bg="#e0f2fe">
                    <Grid cols={2}>
                        ...
                        <Textarea label="Permanent Address" placeholder="Same as current or enter permanent..." value={address.permanent} onChange={set(setAddress)('permanent')} />
                    </Grid>
                    <div style={{ height: 16 }} />
                    <Grid cols={4}>
                ...
                <SectionCard icon={IconFileText} title="Documents" color="#374151" bg="#f3f4f6">
                    <p style={{ fontSize: 13, color: '#9ca3af', marginBottom: 16, margin: '0 0 16px' }}>Upload required documents (PDF, JPG, PNG — max 4MB each)</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                    <button type="button" onClick={() => navigate(-1)} style={{
                        padding: '13px 32px', borderRadius: 12, border: '1.5px solid #e5e7eb',
                        background: 'white', fontWeight: 700, cursor: 'pointer', color: '#6b7280', fontSize: 15
                    }}>Cancel</button>
                    <button type="submit" disabled={isSubmitting || success} style={{
                        ...
                        boxShadow: '0 8px 20px rgba(245,158,11,0.35)', opacity: isSubmitting ? 0.7 : 1
                    }}>
                        {isSubmitting ? <IconLoader2 size={20} className="animate-spin" /> : <IconUserPlus size={20} />}
                        {isSubmitting ? 'Saving Teacher...' : 'Add Teacher'}
                    </button>
                </div>
            </form>
        </div>
