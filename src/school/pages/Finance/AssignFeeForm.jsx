  ...
  return (
    <div className="card soft-card" style={{ padding: 16, marginBottom: 12 }}>
        ...
        <div className="fees-grid">
          {rows.map((row, idx) => (
            <div key={idx} className="fee-row">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <button type="button" onClick={() => removeRow(idx)} className="action-circle"><IconTrash size={16} /></button>
              </div>
            </div>
          ))}
          ...
          <div className="fee-row fee-row-custom">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <button type="button" className="btn-premium btn-premium-outline" style={{ padding: '0.375rem 0.75rem', fontSize: '0.8125rem' }} onClick={addCustom}><IconPlus size={16} /> Add Fee</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 14 }}>
        <button type="button" className="btn-premium btn-premium-outline" onClick={onCancel}>Cancel</button>
        <button type="button" className="btn-premium btn-premium-primary" onClick={handleAssign}>Assign Fees</button>
      </div>
    </div>
  );
};
